    ...
    <li className="relative flex min-h-0 min-w-0 justify-center pl-7 sm:pl-0">
      {!isLast ? (
        <div aria-hidden className="pointer-events-none absolute left-0 right-0 top-0 hidden h-14 sm:block">
          <span className="absolute left-[calc(50%+40px)] top-1/2 h-px w-[calc(100%-80px)] -translate-y-1/2 bg-[#F59E0B]/30" />
        </div>
      ) : null}
      {!isLast ? (
        <span
          aria-hidden
          className="absolute left-[1.72rem] top-[calc(1.75rem+12px)] h-[calc(100%-1.75rem-12px)] w-px bg-[#F59E0B]/30 sm:hidden"
        />
      ) : null}
      <div className="relative flex h-full w-full min-w-0 max-w-[220px] flex-col items-center text-center">
        <div className="relative mb-3 flex h-14 w-14 items-center justify-center rounded-full border border-[#F59E0B]/70 bg-[#F59E0B]/[0.04] shadow-[0_0_0_1px_rgba(245,158,11,0.2),0_0_18px_rgba(245,158,11,0.15)] sm:mb-3.5">
