...
/** Pīlāru virsraksts: tieši divas rindas (augstums = 2× leading; `line-clamp-2` garām tekstiem). */
...
/**
 * Pilnekrāna tumšais Hero: „APPROVED…” + H1 ģeometriski ekrāna centrā;
 * četras ikonas + divrindu virsraksti apakšā; „Turpināt” hero apakšā.
 */
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto flex min-h-0 w-full max-w-[min(100%,53.76rem)] flex-col justify-end">
        <div className={`${homeMarketingPillarGridShellClass} w-full pb-5 pt-4 sm:pb-6 sm:pt-6`}>
          <div
          ...
          >
            {pillars.map((p, i) => {
              ...
              return (
                <article
                  key={`${p.title}-${i}`}
                  className="flex min-h-0 min-w-0 flex-1 basis-0 flex-col items-center gap-2 px-0.5 text-center sm:gap-2.5 sm:px-0.5"
                >
                  <Icon
                    className="h-7 w-7 shrink-0 text-[#0066ff] sm:h-7 sm:w-7 md:h-8 md:w-8"
                    strokeWidth={1.5}
                    aria-hidden
                  />
                  <h3 className={pillarTitleClass}>{p.title}</h3>
                </article>
              );
            })}
          </div>
