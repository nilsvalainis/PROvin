...
import { useMarketingHeroParallax } from "@/components/home/useMarketingHeroParallax";
import {
...
export function MarketingHero() {
  ...
  const { sectionRef, threadRef, glowRef, glassRef, textRef } = useMarketingHeroParallax();
  ...
  return (
    ...
    >
      <div
        ref={threadRef}
        className="home-hero-reflection-thread pointer-events-none absolute inset-0 z-[1] overflow-hidden"
        aria-hidden
      />

      <div className="relative z-[2] mx-auto flex w-full max-w-[min(100%,53.76rem)] flex-col items-center">
        <div className="relative w-full">
          <div
            ref={glowRef}
            className="home-hero-reflection-glow pointer-events-none absolute left-1/2 top-[32%] z-0 h-[min(52vh,26rem)] w-[min(118vw,44rem)] -translate-x-1/2 -translate-y-1/2 rounded-[50%]"
            aria-hidden
          />

          <div ref={glassRef} className="home-hero-glass-panel relative z-[3] rounded-t-2xl px-4 pb-7 pt-6 sm:px-7 sm:pb-8 sm:pt-8">
            <header className="relative z-20 flex w-full shrink-0 flex-col items-center gap-5 text-center sm:gap-7 md:gap-8">
              ...
              <div ref={textRef} className="home-hero-parallax-text w-full">
                <h1
                ...
                >
                  <span className="flex flex-wrap items-center justify-center gap-x-2 gap-y-2 sm:gap-x-2.5 sm:gap-y-2">
                    <span className={`${heroH1BlueKeywordClass} home-hero-h1-blue-polish`}>{t("h1Vin")}</span>
                    <span className="home-hero-h1-silver-polish">{t("h1Un")}</span>
                    <span className={`${heroH1BlueKeywordClass} home-hero-h1-blue-polish`}>{t("h1Sludinajuma")}</span>
                  </span>
                  <span className="home-hero-h1-silver-polish mt-0.5 block sm:mt-1">{t("h1Line2")}</span>
                </h1>
              </div>
              ...
              <div className={`${homeMarketingPillarGridShellClass} mt-7 w-full sm:mt-9`}>
                <div
                  className={`flex w-full flex-row flex-nowrap justify-between gap-3 overflow-x-auto border-b border-white/[0.1] pb-8 [-ms-overflow-style:none] [scrollbar-width:none] sm:gap-5 md:gap-6 [&::-webkit-scrollbar]:hidden ${homeMarketingPillarGridWidthClass}`}
                >
                  {pillars.map((p, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex min-w-0 flex-col items-center gap-2">
                          <h3 className="home-hero-h1-silver-polish text-balance text-[11px] font-semibold uppercase leading-snug tracking-tight sm:text-[13px] md:text-[14px]">
                            {p.title}
                          </h3>
                          {p.body ? (
