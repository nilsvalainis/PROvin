...
import { HeroLiquidMetalBackdrop } from "@/components/home/HeroLiquidMetalBackdrop";
import {
...
/** VARIANT 3 — sudraba gradienta virsraksti (zilie atslēgvārdi atsevišķi). */
const heroLiquidTitleSilverClass =
  "bg-gradient-to-b from-[#fafbfc] via-[#d4dae4] to-[#6e7682] bg-clip-text text-transparent";
const heroLiquidTitleBlueClass =
  "bg-gradient-to-b from-[#82b8ff] via-[#1a7fff] to-[#0040a8] bg-clip-text text-transparent";
...
export function MarketingHero() {
  ...
  return (
    <section
      ref={sectionRef}
      id="home-hero"
      className="relative flex min-h-[100dvh] min-h-[100svh] flex-col justify-center overflow-x-hidden px-4 pb-[max(5.5rem,calc(3.5rem+env(safe-area-inset-bottom,0px)))] pt-[max(1.5rem,env(safe-area-inset-top,0px)+1.25rem)] text-white sm:px-8 sm:pb-20 sm:pt-[max(2.25rem,env(safe-area-inset-top,0px)+1.75rem)]"
      aria-labelledby="marketing-hero-title"
    >
      <HeroLiquidMetalBackdrop sectionRef={sectionRef} />
        ...
        <header className="relative z-20 flex w-full shrink-0 flex-col items-center gap-5 text-center sm:gap-7 md:gap-8">
          ...
          <div className="w-full">
            ...
            >
              <span className="flex flex-wrap items-center justify-center gap-x-2 gap-y-2 sm:gap-x-2.5 sm:gap-y-2">
                <span className={`whitespace-nowrap font-semibold ${heroLiquidTitleBlueClass}`}>{t("h1Vin")}</span>
                <span className={heroLiquidTitleSilverClass}>{t("h1Un")}</span>
                <span className={`whitespace-nowrap font-semibold ${heroLiquidTitleBlueClass}`}>{t("h1Sludinajuma")}</span>
              </span>
              <span className={`mt-0.5 block sm:mt-1 ${heroLiquidTitleSilverClass}`}>{t("h1Line2")}</span>
            </h1>
          </div>
          ...
          <div className="mt-4 flex w-full flex-col items-center sm:mt-5 md:mt-6">
            <Link
              ...
              className="home-hero-cta-liquid provin-btn provin-btn--compact inline-flex w-auto max-w-[min(100%,calc(100vw-2rem))] min-h-12 touch-manipulation items-center justify-center rounded-full bg-[#0066ff] px-6 py-3.5 text-center text-[12px] font-bold uppercase tracking-[0.06em] text-white active:brightness-95 sm:min-h-[3.25rem] sm:px-8 sm:text-[14px] sm:tracking-[0.07em]"
            >
          ...
          <div className={`${homeMarketingPillarGridShellClass} mt-7 w-full sm:mt-9`}>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex min-w-0 flex-col items-center gap-2">
                      <h3
                        className={`text-balance text-[11px] font-semibold uppercase leading-snug tracking-tight sm:text-[13px] md:text-[14px] ${heroLiquidTitleSilverClass}`}
                      >
                        {p.title}
                      </h3>
                      {p.body ? (
            ...
            <div
              className="mx-auto mt-0 h-px w-[min(100%,40rem)] bg-gradient-to-r from-transparent via-white/[0.16] to-transparent"
              aria-hidden
            />
          </div>
        </header>
