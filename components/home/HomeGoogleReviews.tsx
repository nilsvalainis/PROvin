...
import {
  ...
  getFeaturedGoogleReview,
  getGoogleReviewsProfileUrl,
  getSecondaryGoogleReviews,
...
function StarRow({ count = 5, size = "md" }: { count?: number; size?: "md" | "sm" }) {
  const starClass = size === "sm" ? "provin-google-review-star provin-google-review-star--sm" : "provin-google-review-star";
  return (
    <span className="provin-google-review-stars" aria-hidden>
      {Array.from({ length: count }, (_, i) => (
        <svg key={i} viewBox="0 0 20 20" className={starClass}>
          <path
            fill="currentColor"
...
function ReviewCard({ review, featured = false }: { review: GoogleReviewEntry; featured?: boolean }) {
  return (
    <article
      className={featured ? "provin-google-review-card provin-google-review-card--featured" : "provin-google-review-card"}
    >
      ...
      <p className="provin-google-review-card__text">{review.text}</p>
    </article>
  );
}

/** Kurētas Google atsauksmes — bezmaksas statisks bloks. */
export async function HomeGoogleReviews() {
  ...
  const featured = getFeaturedGoogleReview();
  const secondary = getSecondaryGoogleReviews();

  return (
    <section
      id="atsauksmes"
      className="demo-design-dir__section home-body-ink bg-transparent py-16 sm:py-20 md:py-24"
    ...
    >
      <div className="demo-design-dir__shell">
        <header className="text-center">
          ...
          <div className="provin-google-reviews-rating-badge mx-auto mt-4" aria-label={`${GOOGLE_REVIEWS_AGGREGATE_RATING} no 5 zvaigznēm`}>
        ...
        <div className="provin-google-reviews-layout mt-8 sm:mt-10">
          <ReviewCard review={featured} featured />
          <div className="provin-google-reviews-grid">
