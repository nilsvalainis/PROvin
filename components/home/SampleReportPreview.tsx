...
/** Fixed-height PDF viewport: page does not grow; only the PDF scrolls inside. Click chrome → lightbox. */
export function SampleReportPreview({
  ...
  const pdfSrc = href ? `${href}#toolbar=0&navpanes=0&scrollbar=1` : null;
    ...
    <>
      <div className="flex w-full min-w-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-black/35 shadow-[0_12px_40px_-20px_rgba(0,0,0,0.7)]">
        <div className="flex items-center justify-between gap-2 border-b border-white/10 px-3 py-2">
          <p id={titleId} className="min-w-0 truncate text-[0.6875rem] font-semibold uppercase tracking-[0.12em] text-zinc-400">
            {previewLabel}
          </p>
          {href ? (
            <button
