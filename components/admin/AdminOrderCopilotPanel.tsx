...
const SOURCE_TOGGLE_FULL_LABELS: Record<CopilotSourceKey, string> = {
  ...
  finnik: SOURCE_BLOCK_LABELS.finnik,
...
export function AdminOrderCopilotTrigger({
  ...
  return (
    <button
      ...
      className={
        compact
          ? "inline-flex h-7 w-full shrink-0 items-center justify-center rounded-md border border-emerald-200/90 bg-emerald-50/80 px-1 text-emerald-900 shadow-sm transition hover:bg-emerald-100 disabled:opacity-50 dark:border-emerald-800 dark:bg-emerald-950/50 dark:text-emerald-100 dark:hover:bg-emerald-900/40"
    ...
    >
      ...
      {compact ? null : <span className="hidden sm:inline">Copilot</span>}
    </button>
  );
}
