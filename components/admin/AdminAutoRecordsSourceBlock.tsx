...
import { emptyAutoRecordsServiceRow } from "@/lib/admin-source-blocks";
import { autoRecordsRowHasData, parseAutoRecordsPaste, sortAutoRecordsDescending } from "@/lib/auto-records-paste-parse";
...
function GreenCheckIcon() {
  return (
    <span className="inline-flex shrink-0 text-emerald-600" aria-hidden>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col rounded-lg border border-slate-200/90 bg-white p-2 shadow-sm">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        <p className="mb-1 text-[10px] font-semibold uppercase tracking-wide text-[var(--color-provin-muted)]">
          Servisa vēsture
        </p>
        <div className="overflow-x-auto rounded-lg border border-slate-200/90 bg-slate-50/90">
          <table className="w-full min-w-[280px] border-collapse text-[11px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-100/90 text-left text-[10px] font-medium text-[var(--color-provin-muted)]">
                <th className="w-8 px-1 py-1.5" scope="col" />
                <th className="px-2 py-1.5">Datums</th>
                <th className="px-2 py-1.5">Odometrs (km)</th>
                <th className="px-2 py-1.5">Valsts</th>
              </tr>
            </thead>
            <tbody>
              {displayRows.map((row, i) => (
                <tr
                  key={i}
                  className={i % 2 === 0 ? "border-b border-slate-100/80 bg-slate-50/90" : "border-b border-slate-100/80 bg-sky-50/50"}
                >
