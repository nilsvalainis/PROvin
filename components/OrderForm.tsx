...
const inputHero =
  "relative z-10 mt-1.5 box-border min-h-11 w-full rounded-md border border-black bg-black/45 px-3 py-2.5 text-[15px] font-normal text-white outline-none transition-colors placeholder:text-[#b8bcc4]/55 focus:border-[#0066ff] focus:shadow-none focus:ring-0 sm:min-h-0 sm:text-[16px]";
...
export function OrderForm({ className, variant = "default" }: OrderFormProps) {
  ...
  const formShell =
    hero
      ? `space-y-4 rounded-2xl border border-black bg-[#050505]/[0.85] p-5 sm:p-6 ${className ?? ""}`
      : compact
    ...
    <form onSubmit={onSubmit} className={formShell} noValidate>
      ...
      >
        {hero ? (
          <>
            <div
              className="rounded-xl border border-black bg-black/50 px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:px-4"
              role="group"
            ...
            <label
              ...
              className="flex min-h-11 cursor-pointer items-start gap-3 rounded-xl border border-black bg-black/45 px-3 py-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] sm:min-h-0 sm:px-4"
            >
              <input
