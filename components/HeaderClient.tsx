...
export function HeaderClient({
  ...
  const navLinkClass = (active: boolean) =>
    [
      ...
      active ? "text-[#3b82f6]" : "text-white/90 hover:text-white",
    ].join(" ");
    ...
    <>
      <header className="sticky top-0 z-40 border-b border-black/[0.06] bg-white/85 pt-[env(safe-area-inset-top,0px)] backdrop-blur-xl supports-[backdrop-filter]:bg-white/75">
        <div className="mx-auto flex min-h-12 max-w-[980px] items-center justify-between gap-2 pl-[max(1rem,env(safe-area-inset-left,0px))] pr-[max(1rem,env(safe-area-inset-right,0px))] sm:min-h-11 sm:gap-3 sm:px-6 lg:max-w-[1024px]">
          <Link
            href="/"
            className="flex min-h-11 min-w-11 items-center text-[21px] font-semibold tracking-tight text-[#1d1d1f] transition-colors hover:text-provin-accent sm:min-h-0 sm:min-w-0"
          >
            <span>PRO</span>
          ...
          <nav className="hidden min-w-0 items-center gap-6 md:flex md:flex-1 md:justify-end">
            <Link
              href="/"
              className={`text-sm font-medium transition-colors ${isHome && !isFaq && !isOrderSection ? "text-provin-accent" : "text-[#1d1d1f] hover:text-provin-accent"}`}
            >
              {navHome}
            </Link>
            <Link
              href={faqHref}
              className={`text-sm font-medium transition-colors ${isFaq ? "text-provin-accent" : "text-[#1d1d1f] hover:text-provin-accent"}`}
            >
              {faqLabel}
            </Link>
            <Link
              href={orderHref}
              className="provin-btn provin-btn--compact inline-flex min-h-11 shrink-0 items-center justify-center rounded-full px-4 text-[13px] font-semibold shadow-[0_2px_10px_rgba(0,0,0,0.1)] sm:min-h-10 sm:px-5"
            >
              {orderLabel}
            </Link>
          </nav>
          ...
          <div className="flex items-center gap-2 md:hidden">
            <Link
              href={orderHref}
              className="provin-btn provin-btn--compact inline-flex min-h-11 shrink-0 items-center justify-center rounded-full px-4 text-[12px] font-semibold shadow-[0_2px_10px_rgba(0,0,0,0.1)]"
            >
              {orderLabel}
            </Link>
            <button
              ...
              className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full border border-black/[0.08] bg-white text-[#1d1d1f] shadow-sm transition hover:bg-slate-50"
              aria-expanded={open}
