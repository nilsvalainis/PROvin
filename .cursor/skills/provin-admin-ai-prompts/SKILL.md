...
- Omit winter-salt rust typical spots from technical risks or inspection when the exposure brief says OBLIGĀTI (LV/LT/EE years, older car, SUV). Clean/fresh TA and galvanizing do not cancel it. Canonical: `AI_WINTER_SALT_RUST_RULES`.
- Omit the combined paint-thickness / inner-opening section from **2. Ieteikumi** — it is default for every car (one heading + one paragraph). Do not split into three sections or drop other checks to fit it. Canonical: `AI_PAINT_GAUGE_INSPECTION_RULES`.
