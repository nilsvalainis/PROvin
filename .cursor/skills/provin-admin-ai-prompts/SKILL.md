...
- Copy facts from other audits or style-corpus excerpts. Those blocks teach style, vocabulary, and model-family experience only — never wrap, accidents, km, or that other car’s story. Canonical: `AI_HISTORICAL_REPORTS_CONTEXT_RULES`. Self-correction flags `wrap_film_invented` and `foreign_audit_fact_copied`.
- Omit wrap / plēve from technical risks or summary when THIS order already says the car is wrapped. Do not invent wrap from instruction text or other cars’ audits. Canonical: `AI_WRAP_FILM_RULES`.
- Omit winter-salt rust typical spots from technical risks or inspection when the exposure brief says OBLIGĀTI (LV/LT/EE years, older car, SUV). Clean/fresh TA and galvanizing do not cancel it. Do not name Lietuva/Igaunija when THIS car has no link to them. Do not name the tailgate / plate-light rust spot unless the brief says the lid is steel. Canonical: `AI_WINTER_SALT_RUST_RULES`.
- Omit the combined paint-thickness / inner-opening section from **2. Ieteikumi** — it is default for every car (one heading + one paragraph; 100-150 µm, delta 50-150 µm). Do not split into three sections or drop other checks to fit it. Canonical: `AI_PAINT_GAUGE_INSPECTION_RULES`.
- Omit the S-Tronic takeoff / firm-stop, dual-mass flywheel listen (cold and warm, left front wheel), or V6 2012+ cooling leak check before and after a loaded drive when THIS car has those aggregates. Do not copy clutch-disc takeoff onto Tiptronic or V6 cooling onto a 2.0 TDI. Canonical: `AI_TEST_DRIVE_GEARBOX_DMF_RULES`.
- Expand or hedge a source-comment fact („Datu specifika”, „ierakstu trūkums neizslēdz…”, paint-gauge essay). Additions belong only in summary sections. Source comments: 1 paragraph if the facts fit; ceiling 2–3; do not invent a second heading to fill 2–4. Canonical: `AI_SOURCE_FIELDS_FACTS_ONLY_RULES`.
- Recommend replacing a job (zobsiksna, ūdenssūknis, kārbas eļļa) that THIS order’s dealer „Veiktie darbi” / Komentāri already record. Canonical: `AI_DOCUMENTED_SERVICE_WORK_RULES`.
