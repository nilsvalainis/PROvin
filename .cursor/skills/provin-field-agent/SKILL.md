...
- **Epistemic hedging:** PROVIN sees digital data only — not a physical car. Prefer „teorētiski”, „visticamāk”, „ļoti iespējams”, „augsta varbūtība”, „pēc pieejamajiem datiem”, „salīdzinoši labs”, „labs rādījums datos”. Never claim the car is technically perfect or risk-free without in-person inspection. Never pad facts with kancelejisks „signāls/faktors” wording.
...
- **Wrap / plēve:** only if THIS order’s data already says the car is wrapped (not the instruction text, not another audit). Then technical risks and summary must mention the unseen paint work under the film. Canonical: `AI_WRAP_FILM_RULES`.
- **Ziemas sāls / rūsa:** tikai ja **šī** auto datos ir gadi Latvijā / Lietuvā / Igaunijā (īpaši SUV). Ja saistības nav, nenosaukt tās valstis. **1. Tehnisko risku analīze** un **2. Ieteikumi** nosauc tipiskās vietas (arkas, sliekšņi, apakšdaļa). Bagāžnieka vāku pie numura zīmes gaismām nosauc TIKAI ja brīfs saka tērauds. Plastmasa / stiklašķiedra / kompozīts / alumīnijs vai nezināms materiāls: to šablonu NERAKSTI. Cinkojums un svaiga TA to neatceļ. Neizdomā, ka rūsa jau ir. Canonical: `AI_WINTER_SALT_RUST_RULES`.
- **Virsbūves mērījumi:** **2. Ieteikumi** katram auto — viena sadaļa ar krāsas biezuma mērītāju, ap 100 līdz 150 µm, starpību 50 līdz 150 µm un iekšējām ailēm (rūpnīca, ne remonts). Avotu komentāros šo eseju NERAKSTI. Canonical: `AI_PAINT_GAUGE_INSPECTION_RULES`.
- **Avotu lauki = fakti:** per-source „Komentāri” only state what THAT source recorded. No „Datu specifika”, no „neizslēdz iespējamus”, no sentence expansions. Those belong only in summary sections. Canonical: `AI_SOURCE_FIELDS_FACTS_ONLY_RULES`.
- **Testa brauciens (2. Ieteikumi):** 7 pakāpju S-Tronic - lēna uzsākšana pēc bremžu atlaišanas un vieglas gāzes, plus strauja apstāšanās bez grūdieniem. Divmasu spararats - graboņa pie kreisā priekšējā riteņa, aukstam un siltam. Audi V6 TDI ~2012+ noplūdes gan pirms, gan pēc slodzes. Canonical: `AI_TEST_DRIVE_GEARBOX_DMF_RULES`.
- **Dīlera fiksētie darbi (visas markas):** ja Veiktie darbi / Komentāri jau rāda zobsiksnu, ķēdi, ūdenssūkni vai kārbas eļļu, to kā „jāmaina” neraksta. Ķēde pret zobsiksnu tikai pēc šī motora koda vai fiksētas maiņas. Vispirms kods, tad meklēšana šim kodam, tad komentārs. Canonical: `AI_DOCUMENTED_SERVICE_WORK_RULES`, `AI_THIS_CAR_ONLY_LOGIC_RULES`, `AI_POWERTRAIN_IDENTIFICATION_RULES`.

OPERATOR NOTES / PAPILDU PIEZĪMES AI (absolute):
- When the request includes operator notes, they are a BINDING WORK ORDER: process every distinct topic; do not cherry-pick.
- If the operator limited scope („tikai par…”, „neraksti par…”, „nepapildi”), write only those topics — no extra padding. If the notes name a specific source, write that paragraph only in that ACTIVE FIELD. Canonical: `AI_OPERATOR_NOTES_EXECUTION_RULES`.
...
- Oil-change interval math (how often oil was changed, km/time vs manufacturer interval) only in **Eļļas maiņas intervāli**. Other fields: one sentence max if it is a purchase risk. Canonical: `AI_OIL_CHANGE_INTERVAL_RULES`.
...
- **Other audits / style memory:** take only rhythm, vocabulary, and model-family experience. Never transplant another car’s facts. Canonical: `AI_HISTORICAL_REPORTS_CONTEXT_RULES`.
...
- **3. Kopsavilkums:** discussed recorded facts, then ONLY if needed a short conclusion that recommends a workshop / in-person check. Never a dual-history opener, never on-one-hand / on-the-other framing, never unproven claims as verdicts, never a point-by-point rehash of already-generated source/IRISS text, and **never listing/market/repair EUR figures** (those belong in „Cenas vērtējums” and „1. Tehnisko risku analīze”). Also apply `AI_CLIENT_EMAIL_FORMAT_RULES` (plain text, no Markdown in client email).
