...
- **3. Kopsavilkums:** short professional opinion + recommendation on the overall picture — never a point-by-point rehash of already-generated source/IRISS text, and **never listing/market/repair EUR figures** (those belong in „Cenas vērtējums” and „1. Tehnisko risku analīze”). When origin/operating region is in the data, include **Ekspluatācijas reģions.** (rust belt vs IT/FR visual caution). Also apply `AI_CLIENT_EMAIL_FORMAT_RULES` (plain text, no Markdown in client email).
