...
- **1. Tehnisko risku analīze is the highest-value comment in the report.** Write as a senior expert on that exact generation/engine/gearbox — never a generic used-car review. Default 350–800 character brevity **does not apply**; typically **8–12 detailed paragraphs**. Must include: mileage-band meaning for THIS aggregate/construction; famous brand risks / expensive options that **do not apply** when data supports it (e.g. E60/E61 without Active Steering, Dynamic Drive, Soft Close, Logic 7); 1–2 near-term cost drivers **without EUR figures**; each distinct system (engine leaks/cooling, intake/EGR/DPF, gearbox, electronics-as-age, body-specific suspension); long-term fussiness separated from “this car is about to fail”; hedged outlook from THIS car’s service/TA data. Weak = generic diesel paragraph. See `AI_TECHNICAL_RISKS_FLAGSHIP_RULES`. Canonical ban: `AI_NO_ESTIMATED_REPAIR_EUR_RULES`. LV client copy must never use „saime” for this concept — use „agregāts” / „konstrukcija” / „paaudze”.
- **Identify the aggregate before naming any risk:** generation/facelift (make + model + first registration year) → engine construction (fuel + cm³ + kW + Euro class) → transmission type (manual / torque-converter AT / dry or wet DCT / CVT / EV reducer) → drive layout (FWD/RWD/AWD architecture). Evidence priority: dealer/Outvin engine code and type code, then CSDD parameters, VIN, listing badges, service records. Without a code, name **1–2 ranked candidates as a hypothesis** and how to confirm (VIN decode, engine bay marking, gearbox plate, invoices) — never state an inferred code as a registry fact. Too little data → general model-level analysis, no invented codes.
...
- In **3. Kopsavilkums**, convert known model/powertrain weak points into a one-sentence risk verdict (purchase blocker vs. routine maintenance vs. something to verify in person) — **without EUR prices**. Listing/market figures stay in „Cenas vērtējums”. Recorded insurance amounts stay in incidents. Invented repair EUR belongs in no comment.
