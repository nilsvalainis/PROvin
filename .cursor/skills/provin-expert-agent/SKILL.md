...
- Use "automašīna" (or "auto"). NEVER "automobīlis". Never start a paragraph with "- " or "– ". **NEVER** Unicode em dash "—" or en dash "–". Prefer comma, colon, or a new sentence; leftover dashes only ASCII "-". Canonical: `.cursor/rules/no-em-dash.mdc`.
...
- LATVIA / LITHUANIA / ESTONIA (name each country in client copy — never bundle as "Baltics"/"Baltija"): Aggressive rust/corrosion from winter salting, suspension/bushing wear from poor roads, high humidity. If the car has spent years here — especially an SUV — **1. Tehnisko risku analīze** and **2. Ieteikumi** must name typical spots (arches, sill bottoms, underbody). Name the tailgate around plate lights ONLY when THIS car’s lid is steel. Plastic, fiberglass, composite or aluminium lids do not rust there. Galvanized bodies and a clean/fresh TA do not cancel this; TA does not see rust under liners. This is a climate risk, not a proven defect (`AI_WINTER_SALT_RUST_RULES`). Legal risk of fleet/company ownership (VAT fraud checks, weak historic maintenance records). Typically city/short-trip profile — apply strict 10k km oil logic unless dense highway-style mileage data proves otherwise.
...
- **7-speed S-Tronic / dual-clutch:** feel how the car starts after releasing the brake and a light throttle for a slow takeoff. A jerky start can be the first clutch-pack (sajūga disku) sign. After the drive, a firm stop must not leave jerks or knocks. Do not copy this onto Tiptronic.
- **Divmasu spararats** (only if THIS car has one): irregular rattling from the engine bay, best at the left front wheel, on both a cold and a warm engine; vibration on acceleration can also point to DMF wear.
- **Audi / VW 3.0 V6 TDI ~2012+:** oil-in-coolant, thermostat housing, intercooler. Leak check both BEFORE and AFTER a dynamic loaded drive. Canonical: `AI_TEST_DRIVE_GEARBOX_DMF_RULES`.
...
- **Identify the aggregate internally, never as the opening paragraph.** Sequence for every manufacturer: engine code first (dealer/Outvin/CSDD/VIN), then look up THAT code’s construction and typical faults (web search; use a pack only if it names the same code), then write. Generation/engine/gearbox/drive is reasoning, not a client intro. First output paragraph = a risk fact. Without a code, keep 1–2 candidates as a hypothesis next to the relevant risk — never as a registry fact. Too little data → general model-level analysis, no invented codes. Canonical: `AI_POWERTRAIN_IDENTIFICATION_RULES`, `AI_THIS_CAR_ONLY_LOGIC_RULES`.
- **Calibrate every risk to the approximate mileage and age band:** what resource is typically already consumed (must be evidenced in service history), what falls in the next ~20–40k km (the buyer's real cost), what is distant. Prioritize by **probability × EUR**; max **1–2 primary purchase risks**. A 250k km failure mode is not an active threat at 90k km. Age ≠ mileage (rubber, cooling, belts age on time). Documented work lowers the risk: say the records already show it. Missing records are **unproven**, not "not done" - ask the seller for proof. When the picture looks relatively good in the data, say so plainly; fabricated red flags are as damaging as silence.
...
- In **3. Kopsavilkums**, state the discussed recorded facts. Only if a point is not proven in the data, add a short conclusion that recommends a workshop / in-person check. Do not open with a dual-history verdict or on-one-hand framing. Do not claim a defect that the records do not prove. If ANY field says the car is wrapped / aplīmēta, the summary must mention that paint work under the film cannot be judged and that the buyer accepts that risk (`AI_WRAP_FILM_RULES`). Listing/market figures stay in „Cenas vērtējums”. Recorded insurance amounts stay in incidents. Invented repair EUR belongs in no comment.
- In **Ieteikumi klātienes apskatei**, translate remaining uncertainties into specific on-site checks grouped by what the buyer should **see, hear, measure, or ask** (sounds, temperature, shifting, leaks, diagnostics, test-drive maneuvers, battery/HV) — not one paragraph per tech-risk system and not generic “jāpārbauda auto” wording. **Every car** gets one combined paint-thickness section (gauge; ap 100 līdz 150 µm; starpība 50 līdz 150 µm; inner openings often ~half = factory) — raise length (typically 6–12) instead of dropping other checks (`AI_PAINT_GAUGE_INSPECTION_RULES`). Winter-salt typical spots only when the exposure brief is OBLIGĀTI for THIS car’s actual LV/LT/EE use — do not name those countries “just in case”. Historical / style excerpts must be **adapted** to THIS audit and OPERATORA KOMANDAS, never pasted. Generate tech risks first. Sequence for every manufacturer: identify THIS engine code, web-search that code (Motor-Talk, club wikis, independent specialists), then write. Use a pack only if it names the same code. Never invent campaign numbers or pad with generic diesel or TA-covered wear. Never copy another engine’s faults onto this code. If dealer works already record a timing-belt or chain job, do not recommend replacing it.
...
- If the notes name a specific source, that paragraph belongs only in that ACTIVE FIELD; other sources may use it as silent context.
...
- **3. Kopsavilkums** is discussed facts plus, if needed, a workshop-check conclusion — not a recap of every section, not a buy/don't-buy essay, and **never listing/market/repair EUR**.
...
- Per-source „Komentāri” state only what THAT source recorded. One paragraph if the facts fit; ceiling 2–3. No sentence expansions, no „Datu specifika”, no „neizslēdz iespējamus”, no paint-gauge essay. Those additions belong only in summary sections (3. Kopsavilkums, nobraukuma / negadījumu kopsavilkums) or „2. Ieteikumi”. Canonical: `AI_SOURCE_FIELDS_FACTS_ONLY_RULES`.
