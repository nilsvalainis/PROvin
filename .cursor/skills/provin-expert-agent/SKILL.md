...
- **1. Tehnisko risku analīze is the highest-value comment in the report.** Write as a senior expert on that exact generation/engine/gearbox — never a generic used-car review. Default 350–800 character brevity **does not apply**; typically **8–12 detailed paragraphs**. Must include: mileage-band meaning for THIS family; famous brand risks / expensive options that **do not apply** when data supports it (e.g. E60/E61 without Active Steering, Dynamic Drive, Soft Close, Logic 7); 1–2 near-term cost drivers with EUR; each distinct system (engine leaks/cooling, intake/EGR/DPF, gearbox, electronics-as-age, body-specific suspension); long-term fussiness separated from “this car is about to fail”; hedged outlook from THIS car’s service/TA data. Weak = generic diesel paragraph. See `AI_TECHNICAL_RISKS_FLAGSHIP_RULES`.
- **Identify the aggregate before naming any risk:** generation/facelift (make + model + first registration year) → engine family (fuel + cm³ + kW + Euro class) → transmission type (manual / torque-converter AT / dry or wet DCT / CVT / EV reducer) → drive layout (FWD/RWD/AWD architecture). Evidence priority: dealer/Outvin engine code and type code, then CSDD parameters, VIN, listing badges, service records. Without a code, name **1–2 ranked candidates as a hypothesis** and how to confirm (VIN decode, engine bay marking, gearbox plate, invoices) — never state an inferred code as a registry fact. Too little data → family-level analysis, no invented codes.
...
- In **3. Kopsavilkums**, convert known model/powertrain weak points into a one-sentence risk verdict (blocker vs control point) — **without EUR prices**; listing/market/repair figures stay in „Cenas vērtējums” and „1. Tehnisko risku analīze”.
- In **Ieteikumi klātienes apskatei**, translate those same aggregate risks into **6–9** specific on-site checks (sounds, temperature, shifting, leaks, diagnostics, test-drive maneuvers, battery/HV) — one paragraph per tech-risk system, not generic “jāpārbauda auto” wording. Generate tech risks first. If the pack does not cover this exact family, **web-search European owner forums** (Motor-Talk, club wikis, independent specialists) then write the same structure — never invent campaign numbers or pad with generic diesel risks.
- **VW Group 3.0 TDI V6 + 7-speed DSG/S-Tronic (from ~2012, 150/180/190/200/210 kW):** apply `AI_VAG_30TDI_7DSG_RULES` in every field. Spell 150 vs 180 kW. Gearbox **2 000-4 000 €** if takeoff jerks; DMF **700-1 200 €** (idle at front wheels, gone in D/R, before and after drive). Exceptions: **176 kW** (no pronounced chains to honest ~350k) and **Tiptronic 6/8** (normal drive only).
