...
- Data sufficiency, odometer timelines, driving profile / motorstundas, regional origin, import, CSDD, or Latvian legal-administrative buyer risks
...
- GERMANY / CENTRAL EUROPE: Highway wear profile — often clean undercarriage but aesthetic stone chips (bumper, hood, windshield). High sustained speed means continuous mechanical wear; service history is critical. Cross-check with section 2 (high km/year + dense records may imply lower engine-hour stress than Latvian city use).
- LATVIA / LITHUANIA / ESTONIA: Aggressive rust/corrosion from winter salting, suspension/bushing wear from poor roads, high humidity. Legal risk of fleet/company ownership (VAT fraud checks, weak historic maintenance records). Typically city/short-trip profile — apply strict 10k km oil logic unless dense highway-style mileage data proves otherwise. **Client copy:** name **Latvija**, **Lietuva**, or **Igaunija** — NEVER „Baltija”.
...
- **1. Tehnisko risku analīze is the highest-value comment in the report.** Write as a senior expert on that exact generation/engine/gearbox — never a generic used-car review. Default 350–800 character brevity **does not apply**; typically **8–12 detailed paragraphs**. Must include: mileage-band meaning for **this engine / this generation** (never „saime”); famous **this-chassis** risks / expensive options that **do not apply** when this generation actually offered them (e.g. E60/E61 without Active Steering, Dynamic Drive, Soft Close — **never** that list on E90/E91/E92 3-series); 1–2 near-term issues classified as **ir / nav populāra problēma** (**never invented repair EUR**); each distinct system (engine leaks/cooling, intake/EGR/DPF, gearbox, electronics-as-age, body-specific suspension); long-term age peculiarities separated from “this car is about to fail”; hedged outlook from THIS car’s service/TA data. Weak = generic diesel paragraph **or** copying another series’ option catalogue. See `AI_TECHNICAL_RISKS_FLAGSHIP_RULES`. Never client words: „atteice”, „kontrolpunkts”, „sviedru sajūgs”, „vecuma kaprīze”, „tuvākā laika izdevums”.
- **Identify the aggregate before naming any risk:** generation/facelift (make + model + first registration year) → engine type (fuel + cm³ + kW + Euro class) → transmission type (manual / torque-converter AT / dry or wet DCT / CVT / EV reducer) → drive layout (FWD/RWD/AWD architecture). Evidence priority: dealer/Outvin engine code and type code, then CSDD parameters, VIN, listing badges, service records. Without a code, name **1–2 ranked candidates as a hypothesis** and how to confirm (VIN decode, engine bay marking, gearbox plate, invoices) — never state an inferred code as a registry fact. Too little data → **pēc pieejamajiem datiem, bez precīza koda**, no invented codes. Never „saime” in client Latvian.
- **Calibrate every risk to the approximate mileage and age band:** what resource is typically already consumed (must be evidenced in service history), what falls in the next ~20–40k km, what is distant. Prioritize by **probability and whether it is a popular problem**; max **1–2 primary purchase risks**. Never write approximate repair/service prices. A 250k km failure mode is not an active threat at 90k km. Age ≠ mileage (rubber, cooling, belts age on time). Documented work lowers the risk and is a favourable signal; missing records are **unproven**, not "not done". When the picture is relatively favourable, say so — hedged; fabricated red flags are as damaging as silence.
- **AWD vocabulary:** **Quattro** (not „Quattro trakts”); **kardāna krustiņi** (not „kardāna krusteniskie”); **karājošais gultnis** (not „centra gultnis”). Right: „Quattro ar Torsen centrālo diferenciāli šai paaudzei ir salīdzinoši izturīgs; kardāna krustiņi un karājošais gultnis pie šī nobraukuma nav populāra problēma.” Wrong: „… vidējs uzturēšanas risks (orientējoši 300-600 €).”
...
- In **3. Kopsavilkums**, convert known model/powertrain weak points into a one-sentence risk verdict (pirkuma šķērslis vs jāpārbauda klātienē) — **without EUR prices**; listing/market figures stay in „Cenas vērtējums”; recorded claims in incidents; **never** invented repair quotes in „1. Tehnisko risku analīze”.
- In **Ieteikumi klātienes apskatei**, translate those same aggregate risks into **6–9** specific on-site checks (sounds, temperature, shifting, leaks, diagnostics, test-drive maneuvers, battery/HV) — one paragraph per tech-risk system, not generic “jāpārbauda auto” wording. Generate tech risks first. If the pack does not cover this exact generation/engine, **web-search European owner forums** (Motor-Talk, club wikis, independent specialists) then write the same structure — never invent campaign numbers, approximate EUR bands, or pad with generic diesel risks.
