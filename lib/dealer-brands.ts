/**
 * Canonical official-dealer brand coverage for B2C + B2B (+ AZ.VIN demo).
 * Groups follow manufacturer ownership / portfolio order used on the site.
 */
...
/** @deprecated Prefer TP5_DEALER_BRAND_GROUPS — kept as alias for row-style consumers. */
...
export const TP5_DEALER_BRAND_LOGO_SRC: Record<Tp5DealerBrand, string> = {
  ...
  "DS Automobiles": `/brand-logos/ds-automobiles.svg?v=${LOGO_V}`,
...
/** Keep popup / display order aligned with manufacturer groups. */
export function groupTp5DealerBrands(
  brands: readonly string[],
): readonly (readonly string[])[] {
  const want = new Set(brands);
  const grouped = TP5_DEALER_BRAND_GROUPS.map((group) =>
    group.filter((brand) => want.has(brand)),
  ).filter((group) => group.length > 0);
  const known = new Set(TP5_DEALER_BRANDS);
  const extras = brands.filter((brand) => !known.has(brand as Tp5DealerBrand));
  if (extras.length === 0) return grouped;
