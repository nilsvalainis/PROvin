...
describe("BMW i4 dīlera PDF (WBY31AW04NFN09888)", () => {
  it.skipIf(!existsSync(BMW_I4_PDF))("ielasa visus Key Read, kas nav tas pats apmeklējums", async () => {
    ...
    const cbs = extract.serviceHistory.filter((e) => e.category === "CBS nolasījums");
    ...
    expect(cbs.every((e) => e.works.some((w) => /\(līdz \d{2}\.\d{2}\.\d{4}\)/.test(w)))).toBe(true);
    expect(extract.serviceHistory.some((e) => e.date === "10.04.2026" && e.category === "CBS nolasījums")).toBe(
      false,
    );
  });
});
