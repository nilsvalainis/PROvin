...
/**
 * ASV vēstures fotogrāfijas (izsoles / negadījumi) (JPEG — Blob + lokālais disks).
...
import {
  flattenAsvPhotoGroups,
  isAsvPhotoId,
  normalizeAsvPhotoGroups,
} from "@/lib/asv-photo-types";

function identityJpeg(buf: Buffer): Buffer {
  return buf;
}

/** JPEG pēc klienta kompresijas. */
export const ASV_PHOTO_MAX_BYTES = 320 * 1024;

const PHOTO_DIR = "asv-photos";

export function makeAsvPhotoId(): string {
  return `asv_ph_${crypto.randomBytes(12).toString("hex")}`;
}

export function isSafeAsvPhotoId(id: string): boolean {
  return isAsvPhotoId(id);
}
...
export function collectAsvPhotoIdsFromWorkspace(
  workspace: { sourceBlocks?: unknown } | null | undefined,
): Set<string> {
  const keep = new Set<string>();
  const raw = workspace?.sourceBlocks;
  if (!raw || typeof raw !== "object") return keep;
  const block = (raw as { asv?: { photos?: unknown; photoGroups?: unknown } }).asv;
  const groups = normalizeAsvPhotoGroups(block?.photoGroups, block?.photos);
  for (const ph of flattenAsvPhotoGroups(groups)) keep.add(ph.id);
  return keep;
}

export async function writeAsvPhotoJpeg(
  sessionId: string,
  photoId: string,
  jpegBody: Buffer,
): Promise<void> {
  if (!isSafeOrderDraftSessionId(sessionId) || !isSafeAsvPhotoId(photoId)) {
  ...
  if (isBlobPrimary()) {
    ...
    const verify = await readAsvPhotoJpeg(sessionId, photoId);
...
export async function readAsvPhotoJpeg(sessionId: string, photoId: string): Promise<Buffer | null> {
  if (!isSafeOrderDraftSessionId(sessionId) || !isSafeAsvPhotoId(photoId)) return null;
  ...
  if (blob) {
    try {
      ...
      if (res && res.statusCode === 200 && res.stream) {
        ...
        if (isJpegMagicBuffer(buf)) return identityJpeg(buf);
      }
    } catch {
      /* fall through */
    }
  ...
  try {
    ...
    return identityJpeg(buf);
  } catch {
    return null;
  }
}

export async function deleteAsvPhoto(sessionId: string, photoId: string): Promise<void> {
  if (!isSafeOrderDraftSessionId(sessionId) || !isSafeAsvPhotoId(photoId)) return;
...
export async function pruneOrphanAsvPhotos(sessionId: string, keepPhotoIds: Set<string>): Promise<void> {
...
export async function listStoredAsvPhotoIds(sessionId: string): Promise<string[]> {
  if (!isSafeOrderDraftSessionId(sessionId)) return [];
  ...
  if (blob) {
    try {
      ...
      for (const b of blobs) {
        ...
        const id = name.slice(0, -".jpg".length);
        if (isSafeAsvPhotoId(id)) ids.add(id);
      }
    } catch {
      /* ignore */
    }
...
export async function readAsvPhotosForPdf(
  sessionId: string,
  preferredOrder: string[],
): Promise<{ dataUrls: Record<string, string>; missing: string[] }> {
  const stored = await listStoredAsvPhotoIds(sessionId);
  const storedSet = new Set(stored);
  const ordered: string[] = [];
  for (const id of preferredOrder) {
    if (isSafeAsvPhotoId(id) && !ordered.includes(id)) ordered.push(id);
  }
  ...
  for (const id of ordered) {
    const buf = await readAsvPhotoJpeg(sessionId, id);
    if (!buf) {
