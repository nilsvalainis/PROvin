...
export const AUTO_RECORDS_PHOTO_MAX_BYTES = 320 * 1024;

export function makeAutoRecordsPhotoId(): string {
  return `ar_ph_${crypto.randomBytes(12).toString("hex")}`;
}

export function isSafeAutoRecordsPhotoId(id: string): boolean {
  return isAutoRecordsPhotoId(id);
}
...
function listingPhotoBlobPathname(prefix: string, sessionId: string, photoId: string): string {
  ...
  return `${p}auto-records-photos/${sessionId}/${photoId}.jpg`;
}

function listingPhotoFsPath(draftDir: string, sessionId: string, photoId: string): string {
  return path.join(draftDir, "auto-records-photos", sessionId, `${photoId}.jpg`);
}
...
export function collectAutoRecordsPhotoIdsFromWorkspace(
  workspace: { sourceBlocks?: unknown } | null | undefined,
): Set<string> {
  const keep = new Set<string>();
  const raw = workspace?.sourceBlocks;
  if (!raw || typeof raw !== "object") return keep;
  const la = (raw as { auto_records?: { photos?: unknown; photoGroups?: unknown } }).auto_records;
  const groups = normalizeAutoRecordsPhotoGroups(la?.photoGroups, la?.photos);
  for (const ph of flattenAutoRecordsPhotoGroups(groups)) keep.add(ph.id);
  return keep;
}

export async function writeAutoRecordsPhotoJpeg(
  sessionId: string,
  photoId: string,
  jpegBody: Buffer,
): Promise<void> {
  if (!isSafeOrderDraftSessionId(sessionId) || !isSafeAutoRecordsPhotoId(photoId)) {
  ...
  if (isListingPhotoBlobPrimary()) {
    ...
    const verify = await readAutoRecordsPhotoJpeg(sessionId, photoId);
...
export async function readAutoRecordsPhotoJpeg(
  sessionId: string,
  photoId: string,
): Promise<Buffer | null> {
  if (!isSafeOrderDraftSessionId(sessionId) || !isSafeAutoRecordsPhotoId(photoId)) return null;
...
export async function deleteAutoRecordsPhoto(sessionId: string, photoId: string): Promise<void> {
  if (!isSafeOrderDraftSessionId(sessionId) || !isSafeAutoRecordsPhotoId(photoId)) return;
...
export async function pruneOrphanAutoRecordsPhotos(
  sessionId: string,
    ...
    try {
      ...
      await Promise.all(
        blobs.map(async (b) => {
          ...
          if (!isSafeAutoRecordsPhotoId(id)) return;
  ...
  const draftDir = getOrderDraftStorageDir();
  if (!draftDir) return;
  const dir = path.join(draftDir, "auto-records-photos", sessionId);
  let names: string[];
  try {
...
export async function listStoredAutoRecordsPhotoIds(sessionId: string): Promise<string[]> {
  if (!isSafeOrderDraftSessionId(sessionId)) return [];
  ...
  if (blob) {
    try {
      ...
      for (const b of blobs) {
        ...
        const id = name.slice(0, -".jpg".length);
        if (isSafeAutoRecordsPhotoId(id)) ids.add(id);
      }
    } catch {
      /* ignore */
    }
  ...
  if (draftDir && !shouldSkipEphemeralListingPhotoFs()) {
    try {
      const names = await fs.readdir(path.join(draftDir, "auto-records-photos", sessionId));
...
export async function readAutoRecordsPhotosForPdf(
  sessionId: string,
  preferredOrder: string[],
): Promise<{ dataUrls: Record<string, string>; missing: string[] }> {
  const stored = await listStoredAutoRecordsPhotoIds(sessionId);
  const storedSet = new Set(stored);
  const ordered: string[] = [];
  for (const id of preferredOrder) {
    if (isSafeAutoRecordsPhotoId(id) && !ordered.includes(id)) ordered.push(id);
  }
  ...
  for (const id of ordered) {
    const buf = await readAutoRecordsPhotoJpeg(sessionId, id);
    if (!buf) {
