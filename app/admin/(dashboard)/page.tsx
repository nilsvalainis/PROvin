  ...
  return (
    <div className="mx-auto max-w-5xl">
      <header className="mb-8">
        <h1 className="text-2xl font-semibold tracking-tight text-[var(--color-apple-text)]">
          Pasūtījumi
        </h1>
        <p className="mt-1 text-sm text-[var(--color-provin-muted)]">
          Apmaksātie pasūtījumi no Stripe Checkout. Izvēlies rindu, lai atvērtu detaļas un turpinātu
          apstrādi.
          {demoOn ? (
            <>
              {" "}
              <span className="font-medium text-[var(--color-provin-accent)]">
                Demo pasūtījumi
              </span>{" "}
              ir ieslēgti (<code className="rounded bg-slate-100 px-1 text-xs">ADMIN_DEMO_ORDERS</code>
              ).
            </>
          ) : null}
        </p>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50/80 text-xs font-semibold uppercase tracking-wide text-[var(--color-provin-muted)]">
                  <th className="px-4 py-3">Datums</th>
                  <th className="px-4 py-3">VIN</th>
                  <th className="px-4 py-3">Klients</th>
                  <th className="px-4 py-3 text-right">Summa</th>
                  <th className="px-4 py-3 text-right">Darbība</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {orders.map((o) => (
                  ...
                  >
                    <td className="whitespace-nowrap px-4 py-3 text-[var(--color-apple-text)]">
                    ...
                    <td className="px-4 py-3 font-mono text-xs text-[var(--color-apple-text)]">
                      {o.vin ?? "—"}
                    </td>
                    <td className="max-w-[200px] truncate px-4 py-3 text-[var(--color-apple-text)]">
                      {o.customerEmail ?? "—"}
                    </td>
                    <td className="whitespace-nowrap px-4 py-3 text-right tabular-nums text-[var(--color-apple-text)]">
                      {formatMoneyEur(o.amountTotal, o.currency)}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <Link
                        href={`/admin/orders/${encodeURIComponent(o.id)}`}
                        className="inline-flex rounded-full bg-[var(--color-provin-accent-soft)] px-3 py-1.5 text-xs font-medium text-[var(--color-provin-accent)] hover:bg-[#d4e8fb]"
                      >
                        Atvērt
                      </Link>
                    </td>
