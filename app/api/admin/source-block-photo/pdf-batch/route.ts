/**
 * PDF: visu avotu sadaļu fotogrāfiju base64 no ilgtermiņa glabātuves.
 */
  ...
  try {
    ...
    if (Array.isArray(b.photoIds)) {
      preferredOrder = b.photoIds
        ...
        .map((id) => id.trim())
        .filter((id) => isSourceBlockPhotoId(id));
    }

    if (preferredOrder.length === 0) {
      const draft = await readOrderDraft(sessionId);
      preferredOrder = [...collectSourceBlockPhotoIdsFromWorkspace(draft?.workspace ?? null)];
    }

    const { dataUrls, missing } = await readSourceBlockPhotosForPdf(sessionId, preferredOrder);
    ...
    console.error("[source-block-photo/pdf-batch]", msg);
