...
import {
  SOURCE_BLOCK_PHOTO_MAX_BYTES,
  collectSourceBlockPhotoIdsFromWorkspace,
  deleteSourceBlockPhoto,
  isSafeSourceBlockPhotoId,
  makeSourceBlockPhotoId,
...
import { SOURCE_BLOCK_MAX_PHOTOS } from "@/lib/source-block-photo-types";
...
async function persistedPhotoCount(sessionId: string): Promise<number> {
  ...
  return collectSourceBlockPhotoIdsFromWorkspace(draft?.workspace ?? null).size;
}
...
export async function GET(req: Request) {
  try {
    ...
    if (!isSafeSourceBlockPhotoId(photoId)) {
    ...
    const buf = await readSourceBlockPhotoJpeg(sessionId, photoId);
    ...
    console.error("[source-block-photo] GET", e);
...
export async function POST(req: Request) {
  try {
    ...
    const blockCount = Number.isFinite(clientCount) && clientCount >= 0 ? clientCount : 0;
    if (blockCount >= SOURCE_BLOCK_MAX_PHOTOS || persisted >= SOURCE_BLOCK_MAX_PHOTOS * 12) {
      return NextResponse.json({ error: "photo_limit" }, { status: 400 });
    }

    const ab = await file.arrayBuffer();
    const normalized = await jpegFromAdminPhotoUpload(Buffer.from(ab), SOURCE_BLOCK_PHOTO_MAX_BYTES);
    if (!normalized.ok) {
      return NextResponse.json({ error: normalized.error }, { status: 400 });
    }
    const buf = normalized.jpeg;

    const photoId = makeSourceBlockPhotoId();
    try {
      await writeSourceBlockPhotoJpeg(sessionId, photoId, buf);
    } catch (e) {
      const detail = e instanceof Error ? e.message : String(e);
      console.error("[source-block-photo] POST write", sessionId, detail);
    ...
    console.error("[source-block-photo] POST", msg);
  ...
  try {
    ...
    const photoIds =
      Array.isArray(photoIdsRaw) && photoIdsRaw.length > 0
        ? photoIdsRaw.filter((id): id is string => typeof id === "string" && isSafeSourceBlockPhotoId(id.trim()))
        : [];
    ...
    if (deleteAll) {
      const draft = await readOrderDraft(sessionId);
      const ids = [...collectSourceBlockPhotoIdsFromWorkspace(draft?.workspace ?? null)];
      await Promise.all(ids.map((id) => deleteSourceBlockPhoto(sessionId, id)));
      return NextResponse.json({ ok: true, deleted: ids.length });
    }

    if (photoIds.length > 0) {
      await Promise.all(photoIds.map((id) => deleteSourceBlockPhoto(sessionId, id.trim())));
    ...
    await deleteSourceBlockPhoto(sessionId, photoId);
    return NextResponse.json({ ok: true, deleted: 1 });
  } catch (e) {
    console.error("[source-block-photo] DELETE", e);
    return NextResponse.json({ error: "server_error" }, { status: 500 });
  }
}
