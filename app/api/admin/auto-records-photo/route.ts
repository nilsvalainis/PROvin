/**
 * Oficiālā dīlera fotogrāfijas (JPEG — Blob + lokālais disks).
 */
import { NextResponse } from "next/server";

import { getAdminSession } from "@/lib/admin-auth";
import {
  collectAutoRecordsPhotoIdsFromWorkspace,
  deleteAutoRecordsPhoto,
  isJpegMagicBuffer,
  isSafeAutoRecordsPhotoId,
  AUTO_RECORDS_PHOTO_MAX_BYTES,
  makeAutoRecordsPhotoId,
  readAutoRecordsPhotoJpeg,
...
import { AUTO_RECORDS_MAX_PHOTOS } from "@/lib/auto-records-photo-types";
...
async function persistedPhotoCount(sessionId: string): Promise<number> {
  ...
  return collectAutoRecordsPhotoIdsFromWorkspace(draft?.workspace ?? null).size;
}
...
export async function GET(req: Request) {
  try {
    ...
    if (!isSafeAutoRecordsPhotoId(photoId)) {
    ...
    const buf = await readAutoRecordsPhotoJpeg(sessionId, photoId);
    ...
    console.error("[auto-records-photo] GET", e);
...
export async function POST(req: Request) {
  try {
    ...
    if (effectiveCount >= AUTO_RECORDS_MAX_PHOTOS) {
      return NextResponse.json({ error: "photo_limit" }, { status: 400 });
    }

    const ab = await file.arrayBuffer();
    const buf = Buffer.from(ab);
    if (buf.length === 0 || buf.length > AUTO_RECORDS_PHOTO_MAX_BYTES) {
    ...
    try {
      await writeAutoRecordsPhotoJpeg(sessionId, photoId, buf);
    } catch (e) {
      const detail = e instanceof Error ? e.message : String(e);
      console.error("[auto-records-photo] POST write", sessionId, detail);
    ...
    console.error("[auto-records-photo] POST", msg);
  ...
  try {
    ...
    const photoIds =
      Array.isArray(photoIdsRaw) && photoIdsRaw.length > 0
        ? photoIdsRaw.filter((id): id is string => typeof id === "string" && isSafeAutoRecordsPhotoId(id.trim()))
        : [];
    ...
    if (deleteAll) {
      const draft = await readOrderDraft(sessionId);
      const ids = [...collectAutoRecordsPhotoIdsFromWorkspace(draft?.workspace ?? null)];
      await Promise.all(ids.map((id) => deleteAutoRecordsPhoto(sessionId, id)));
      return NextResponse.json({ ok: true, deleted: ids.length });
    }

    if (photoIds.length > 0) {
      await Promise.all(photoIds.map((id) => deleteAutoRecordsPhoto(sessionId, id.trim())));
    ...
    await deleteAutoRecordsPhoto(sessionId, photoId);
    return NextResponse.json({ ok: true, deleted: 1 });
  } catch (e) {
    console.error("[auto-records-photo] DELETE", e);
    return NextResponse.json({ error: "server_error" }, { status: 500 });
  }
}
