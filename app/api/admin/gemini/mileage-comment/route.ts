/**
 * Admin: Gemini — NOBRAUKUMA VĒSTURES KOMENTĀRS (tikai DEMO pasūtījumi).
 */
...
export async function POST(req: Request) {
  ...
  try {
    const text = await generateMileageCommentWithGemini(parseGeminiOrderContextFromBody(b, sourceBlocks));
    return NextResponse.json({ text });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "unknown";
    console.error("[gemini/mileage-comment]", msg);
