/**
 * Admin: Gemini — Tehnisko risku analīze (APPROVED BY IRISS, 1. sadaļa).
 */
import { NextResponse } from "next/server";

import { getAdminSession } from "@/lib/admin-auth";
import { assertGeminiAllowedForSession } from "@/lib/admin-gemini-demo-guard";
import { generateTechnicalRiskAnalysisWithGemini } from "@/lib/admin-gemini-technical-risks";
...
export async function POST(req: Request) {
  ...
  try {
    const text = await generateTechnicalRiskAnalysisWithGemini(parseGeminiOrderContextFromBody(b, sourceBlocks));
    return NextResponse.json({ text });
  } catch (e) {
    const msg = e instanceof Error ? e.message : "unknown";
    console.error("[gemini/technical-risk-analysis]", msg);
