/**
 * ASV vēstures fotogrāfijas (JPEG — Blob + lokālais disks).
 */
...
import {
  ASV_PHOTO_MAX_BYTES,
  collectAsvPhotoIdsFromWorkspace,
  deleteAsvPhoto,
  isSafeAsvPhotoId,
  makeAsvPhotoId,
  readAsvPhotoJpeg,
...
import { ASV_MAX_PHOTOS } from "@/lib/asv-photo-types";
...
async function persistedPhotoCount(sessionId: string): Promise<number> {
  ...
  return collectAsvPhotoIdsFromWorkspace(draft?.workspace ?? null).size;
}
...
export async function GET(req: Request) {
  try {
    ...
    const buf = await readAsvPhotoJpeg(sessionId, photoId);
...
export async function POST(req: Request) {
  try {
    ...
    if (effectiveCount >= ASV_MAX_PHOTOS) {
    ...
    try {
      await writeAsvPhotoJpeg(sessionId, photoId, buf);
    } catch (e) {
      const detail = e instanceof Error ? e.message : String(e);
      console.error("[asv-photo] POST write", sessionId, detail);
    ...
    console.error("[asv-photo] POST", msg);
  ...
  try {
    ...
    const photoIds =
      Array.isArray(photoIdsRaw) && photoIdsRaw.length > 0
        ? photoIdsRaw.filter((id): id is string => typeof id === "string" && isSafeAsvPhotoId(id.trim()))
        : [];
    ...
    if (deleteAll) {
      const draft = await readOrderDraft(sessionId);
      const ids = [...collectAsvPhotoIdsFromWorkspace(draft?.workspace ?? null)];
      await Promise.all(ids.map((id) => deleteAsvPhoto(sessionId, id)));
      return NextResponse.json({ ok: true, deleted: ids.length });
    }

    if (photoIds.length > 0) {
      await Promise.all(photoIds.map((id) => deleteAsvPhoto(sessionId, id.trim())));
    ...
    await deleteAsvPhoto(sessionId, photoId);
    return NextResponse.json({ ok: true, deleted: 1 });
  } catch (e) {
    console.error("[asv-photo] DELETE", e);
    return NextResponse.json({ error: "server_error" }, { status: 500 });
  }
}
