/**
 * PDF: negadījumu fotogrāfiju base64 no ilgtermiņa glabātuves (Blob / disks).
 */
  ...
  try {
    ...
    if (Array.isArray(b.photoIds)) {
      preferredOrder = b.photoIds
        ...
        .map((id) => id.trim())
        .filter((id) => isIncidentPhotoId(id));
    }

    if (preferredOrder.length === 0) {
      const draft = await readOrderDraft(sessionId);
      preferredOrder = [...collectIncidentPhotoIdsFromWorkspace(draft?.workspace ?? null)];
    }

    const { dataUrls, missing } = await readIncidentPhotosForPdf(sessionId, preferredOrder);
    ...
    console.error("[incident-photo/pdf-batch]", msg);
