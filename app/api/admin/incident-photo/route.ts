/**
 * Negadījumu kopsavilkuma fotogrāfijas (JPEG — Blob + lokālais disks).
 */
...
import { INCIDENT_MAX_PHOTOS } from "@/lib/incident-photo-types";
...
async function persistedPhotoCount(sessionId: string): Promise<number> {
  ...
  return collectIncidentPhotoIdsFromWorkspace(draft?.workspace ?? null).size;
}
...
export async function GET(req: Request) {
  try {
    ...
    if (!isSafeIncidentPhotoId(photoId)) {
    ...
    const buf = await readIncidentPhotoJpeg(sessionId, photoId);
    ...
    console.error("[incident-photo] GET", e);
...
export async function POST(req: Request) {
  try {
    ...
    if (effectiveCount >= INCIDENT_MAX_PHOTOS) {
    ...
    try {
      await writeIncidentPhotoJpeg(sessionId, photoId, buf);
    } catch (e) {
      const detail = e instanceof Error ? e.message : String(e);
      console.error("[incident-photo] POST write", sessionId, detail);
    ...
    console.error("[incident-photo] POST", msg);
  ...
  try {
    ...
    const photoIds =
      Array.isArray(photoIdsRaw) && photoIdsRaw.length > 0
        ? photoIdsRaw.filter((id): id is string => typeof id === "string" && isSafeIncidentPhotoId(id.trim()))
        : [];
    ...
    if (deleteAll) {
      const draft = await readOrderDraft(sessionId);
      const ids = [...collectIncidentPhotoIdsFromWorkspace(draft?.workspace ?? null)];
      await Promise.all(ids.map((id) => deleteIncidentPhoto(sessionId, id)));
      return NextResponse.json({ ok: true, deleted: ids.length });
    }

    if (photoIds.length > 0) {
      await Promise.all(photoIds.map((id) => deleteIncidentPhoto(sessionId, id.trim())));
    ...
    await deleteIncidentPhoto(sessionId, photoId);
    return NextResponse.json({ ok: true, deleted: 1 });
  } catch (e) {
    console.error("[incident-photo] DELETE", e);
    return NextResponse.json({ error: "server_error" }, { status: 500 });
  }
}
